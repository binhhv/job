...
class Job_province extends CI_Controller {
	...
	function index($provinceName, $id) {
		...
		$head = $this->load->view('main/head', array('styleJob' => $styleJob, 'scriptJob' => $scriptJob, 'user' => $user, 'titlePage' => 'JOB7VN Group|Contact'), TRUE);
		...
		$centerMap = json_encode($province);
		$jobMap = json_encode($this->job->getAllRecruitmentForMap());
		//$jobSames = $this->job->getSameRecruitment($job);
		$content = $this->load->view('main/job/job-province', array('centerMap' => $centerMap, 'jobMap' => $jobMap, 'searchHorizontal' => $searchHorizontal, 'province' => $province, 'listJob' => $listRecruitmentProvincePage, 'page' => $page, 'pagination' => $pagination, 'numjob' => count($listRecruitmentProvince)), TRUE);
