...
class Employer_recruitment extends CI_Controller {
	...
	function updateRecruitment() {
		...
		if ($this->form_validation->run()) {
			...
			$employer_id = $this->security->xss_clean($this->input->post('employer_id'));
			$welfare = $this->security->xss_clean($this->input->post('welfare'));
			$idRec = $this->security->xss_clean($this->input->post('idRec'));
			log_message('error', 'id rec' . $idRec);
			$time_work_insert = strtotime($time_work);
			$user = $this->session->userdata['user'];
			$data = array(
				'rec_title' => $title_recruitment,
	...
	function draftUpdateRecruitment() {
		$title_recruitment = $this->security->xss_clean($this->input->post('title_recruitment'));
		$province = $this->security->xss_clean($this->input->post('province'));
		$time_work = $this->security->xss_clean($this->input->post('time_work'));
		$content_recruitment = $this->security->xss_clean($this->input->post('content_recruitment'));
	...
	function deleteRecruitment() {
		...
		if (is_numeric($idRec)) {
			...
			if ($result) {
				echo json_encode(array('content' => array('status' => 'success', 'modal' => 'modal-delete-recruitment-em', 'csrf' => $csrf)));
			} else {
				echo json_encode(array('content' => array('status' => 'error', 'modal' => 'modal-delete-recruitment-em', 'csrf' => $csrf)));
			}
		} else {
			redirect(base_url('404'), 'refresh');
		}
	...
	function getViewRecruitment() {
		...
		$view = $this->load->view('main/employer/partial/recruitments', array('listRecruitment' => $listRecruitment, 'employerInfo' => $employerInfo), TRUE);
		echo $view;
	}
	...
	function index() {
		...
		$listRecruitment = $this->recruitment->getListRecruitmentEmployer($employerInfo->employer_id);
		$breadcrumb = $this->load->view('main/employer/breadcrumb', array('breadcrumbs' => $breadcrumbs), TRUE);
		$contentEmployer = $this->load->view('main/employer/recruitments', array('csrf' => $csrf, 'employerInfo' => $employerInfo, 'listRecruitment' => $listRecruitment), TRUE);
		$content = $this->load->view('main/employer/layout', array('contentEmployer' => $contentEmployer, 'update_account_employer' => $update_account_employer,
			'employer_menu' => $employer_menu, 'breadcrumb' => $breadcrumb), TRUE);
		$footer = $this->load->view('main/footer', array(), TRUE);
		$this->load->view('main/layout', array('head' => $head, 'header' => $header, 'content' => $content, 'footer' => $footer, 'isGray' => true));
	...
	function detailRecruitment($idRec) {
		$user = $this->session->userdata['user'];
		$employerInfo = $this->employer->getInfoEmployer($user['id']);
		$numRecruitmentPub = $this->employer->getNumRecruitmentAccout($employerInfo->employer_id, $user['id']);
		$employerInfo->numRecruitmentAccount = $numRecruitmentPub;
		...
		$employerInfo->csrf = $csrf;
		if (intval($idRec)) {
			$recruitment = $this->recruitment->getDetailRecruitment($idRec);
		} else {
			$recruitment = null;
		}
		$update_account_employer = $this->load->view('main/employer/modal_update_account_employer', array('employerInfo' => $employerInfo, 'csrf' => $csrf), TRUE);
		...
		$head = $this->load->view('main/head', array('title' => (isset($recruitment->rec_title)) ? urldecode($recruitment->rec_title) : '', 'scriptOption' => $scriptOption, 'styleOption' => $styleOption, 'scriptTable' => true), TRUE);
		$header = $this->load->view('main/header', array('user' => $user, 'showTitle' => true,
		), TRUE);
		//get list account of employer
		//$listAccount = $this->employer->getListAccount($employerInfo->employer_id);
		...
		//$listRecruitment = $this->recruitment->getListRecruitmentEmployer($employerInfo->employer_id);
		$breadcrumb = $this->load->view('main/employer/breadcrumb', array('breadcrumbs' => $breadcrumbs), TRUE);
		$contentEmployer = $this->load->view('main/employer/detail-recruitment', array('csrf' => $csrf, 'employerInfo' => $employerInfo, 'recruitment' => $recruitment), TRUE);
		$content = $this->load->view('main/employer/layout', array('contentEmployer' => $contentEmployer, 'update_account_employer' => $update_account_employer,
			'employer_menu' => $employer_menu, 'breadcrumb' => $breadcrumb), TRUE);
		$footer = $this->load->view('main/footer', array(), TRUE);
		$this->load->view('main/layout', array('head' => $head, 'header' => $header, 'content' => $content, 'footer' => $footer, 'isGray' => true));
	...
	function getViewEditRecruitment($idRec) {
		$user = $this->session->userdata['user'];
		$employerInfo = $this->employer->getInfoEmployer($user['id']);
		$employerInfo->user = $user;
		$csrf = array(
		...
		if (intval($idRec)) {
			$recruitment = $this->recruitment->getDetailRecruitment($idRec);
			$welfareRecruitment = $this->recruitment->getListWelfareRecruitment($idRec);
			$provinceRecruitment = $this->recruitment->getListProvinceRecruitment($idRec, $recruitment->rec_job_map_country);
		} else {
			$recruitment = null;
			$welfareRecruitment = null;
			$provinceRecruitment = null;
		}
		...
		$head = $this->load->view('main/head', array('title' => 'Sửa đổi tuyển dụng', 'scriptOption' => $scriptOption, 'styleOption' => $styleOption), TRUE);
		$header = $this->load->view('main/header', array('user' => $user, 'showTitle' => true,
		), TRUE);
		//get province country

		$contentEmployer = $this->load->view('main/employer/edit-recruitment', array('csrf' => $csrf, 'employerInfo' => $employerInfo,
			'provinceVN' => $provinceVN, 'provinceJP' => $provinceJP,
			'listCareer' => $listCareer, 'listSalary' => $listSalary,
			'listWelfare' => $listWelfare, 'listJobForm' => $listJobForm,
			'listJobFormChild' => $listJobFormChild, 'listLevelJP' => $listLevelJP,
			'listContactForm' => $listContactForm, 'recruitment' => $recruitment,
