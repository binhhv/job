...
class Admin extends CI_Controller {
	...
	public function loadViewAdmin() {
		$numRecruitment = $this->api->getNumRecruitment();
		$numEmployer = $this->api->getNumEmployer();
		$numJobseeker = $this->api->getNumJobseeker();
		$numDoc = $this->api->getNumCVDocument();
		$recentRecruitment = $this->api->getRecentRecruitment();
		$recentUser = $this->api->getRecentJobseekerAndEmployer();
		$scripts = array(
			"assets/admin/dist/js/admin-chart.js");

		$head = $this->load->view('admin/head', array(), TRUE);
		$header = $this->load->view('admin/header', array(), TRUE);
		$content = $this->load->view('admin/content', array(
			'numRecruitment' => $numRecruitment,
			'numEmployer' => $numEmployer,
			'numJobseeker' => $numJobseeker,
			'numDoc' => $numDoc,
			'recentRecruitment' => $recentRecruitment,
			'recentUser' => $recentUser), TRUE);
	...
	function createJobseeker() {
		...
		//var_dump($ouput);
		$code = $this->util->General_Code('account', $output, 4);
		$outputupdate = $this->util->update_Code('account', 'account_code', $code, 'account_id', $output);
