  ...
  <div id="faq-ctrl"  ng-controller="faqController">
...
<section class="content-header">
        ...
        <section class="content-header">
          <div class="row">
            ...
            <div class="col-md-12">
              <button class="btn btn-primary pull-right reload-province" ng-click="reloadFAQ();" >tải lại dữ liệu</button></div>
          </div>
        </section>
            ...
            <div class="col-xs-12">
              <div class="box province-box">
                <div class="box-header">
                  <div class="col-sm-6 col-xs-12">  <button class="btn btn-sm btn-info" ng-click="openModalCreateFAQ('md')">tạo mới</button></div>
                ...
                <div class="box-body table-responsive no-padding"  ng-show="filteredItems > 0">
                  <table class="table table-hover table-striped hide" id="faqTable" ng-init="getFAQ();">
                    <tr>
                    ...
                    <tr ng-repeat="data in filtered = (pagedItems | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit">
                      <td class="text-center">{{data.faq_question | cut:true:50:' ...'}}</td><!--{{($index + ((currentPage -1)* entryLimit)) + 1}}-->
                       <td class="text-center">{{data.faq_answer | cut:true:50:' ...'}}</td>
                      <td class="text-center">{{formatDate(data.faq_created_at) | date: "dd/MM/yyyy"}}</td>


                      <td class="text-center">
                      <button class="btn btn-xs btn-warning" ng-click="openModalUpdateFAQ('md',data)" >sửa</button>
                      <button class="btn btn-xs btn-danger" ng-click="openModalDeleteFAQ('md',data)">xóa</button>
                      </td>
                    </tr>
