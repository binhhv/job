...
class Blog_model extends CI_Model {
	...
	function getListBlogCurrent() {
		$sql = "select * from blog where blog_is_delete = 0 and blog_is_active = 1
		order by blog_created_at desc limit 0,10 ";
		return $this->dbutil->getFromDbQueryBinding($sql, array());
	}
	function getListBlogCategoryCurrent($category) {
		$sql = "select * from blog where blog_is_delete = 0 and blog_is_active = 1 and blog_map_category = ?
		order by blog_created_at desc limit 0,10 ";
		return $this->dbutil->getFromDbQueryBinding($sql, array($category));
	}
	function getListBlogPagging($perpage = 0, $page = 0) {
		$queryLimit = '';
		if ($perpage > 0 || $page > 0) {
		...
		left join account as c on c.account_id = a.blog_map_account
		where blog_is_delete = 0
		order by blog_created_at desc " . $queryLimit;
		return $this->dbutil->getFromDbQueryBinding($sql, array());
	}
	function getListBlogCategoryPagging($category, $perpage = 0, $page = 0) {
		$queryLimit = '';
		if ($perpage > 0 || $page > 0) {
			$queryLimit .= ' limit ' . $page . ', ' . $perpage; //'. $page . ', ' . $perpage';
		}
		...
		left join blog_category as b on b.cblog_id = a.blog_map_category and b.cblog_is_delete = 0
		left join account as c on c.account_id = a.blog_map_account
		where blog_is_delete = 0 and blog_map_category = ?
		order by blog_created_at desc " . $queryLimit;
		return $this->dbutil->getFromDbQueryBinding($sql, array($category));
	}
}