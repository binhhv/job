...
class Job_model extends CI_Model {
	...
	public function getListCenterMapFromRecruitment($idrecruitment) {
		$sql = "select a.*,b.*
				from recruitment_map_province a
				left join province b on b.province_id = a.recmp_map_province  and b.province_is_delete = 0
				where a.recmp_is_delete = 0 and  a.recmp_map_rec = " . $idrecruitment;
	...
	function getSameRecruitment($idrecruitment) {
		...
		// 	   				where a.rec_is_delete = 0 and a.rec_is_disabled = 0 and a.rec_is_approve = 1 ) as c
		// 	   	 on c.rec_id = recmp_map_rec
		//                left join ( select e.*,career.*,salary.*
		//                           from recruitment e
		//                           left join career on career_id = e.rec_job_map_career and career_is_delete = 0
		//                           left join salary on salary_id = e.rec_map_salary and salary_is_delete = 0
		//                           where e.rec_is_delete = 0
		//                           group by e.rec_id
		//                     ) as d on d.rec_id = recmp_map_rec and d.rec_is_delete = 0
		// 	   	 where recmp_map_province in
		// 	   	 (select recmp_map_province
		// 	   	 	from recruitment_map_province
		// 	   	 	where recmp_map_rec = ? and recmp_is_delete = 0)
		// 		and recmp_map_rec <> ? order by recmp_created_at desc limit 10";
		$sql = "select distinct(recmp_map_rec) ,
					  c.employer_name,c.rec_title,d.*,h.province_name
			   from recruitment_map_province
			   left join ( select a.rec_id,b.employer_name,a.rec_title
			   				from recruitment a
			   				left join employer b
                 ...
                 left join province h on province_id = recmp_map_province and province_is_delete = 0
			   	 where recmp_map_province in
			   	 (select recmp_map_province
			   	 	from recruitment_map_province
			   	 	where recmp_map_rec = ? and recmp_is_delete = 0)
	...
	function getCenterMap($location) {
		// $sql = "select a.*,b.*
		// 		from recruitment_map_province a
		// 		left join province b on b.province_id = a.recmp_map_province  and b.province_is_delete = 0
		// 		where a.recmp_is_delete = 0 and  a.recmp_map_rec = " . $idrecruitment;
		...
		if ($location != 0) {
			$sql = "select * from province where province_is_delete = 0 and province_id =" . $location;
			return $this->dbutil->getOneRowQueryFromDb($sql, array());
		} else {
			$sql = "select *, count(recmp_map_rec) as numrec
					from recruitment_map_province
					left join province on province_id = recmp_map_province and province_is_delete = 0
					where recmp_is_delete = 0
					group by recmp_map_province
	...
	function getAllRecruitmentRegion($region) {
		$sql = "select a.*,b.*,count(a.recmp_id) as numjob
				from recruitment_map_province a
				left join province b on b.province_id = a.recmp_map_province  and b.province_is_delete = 0
                left join recruitment c on c.rec_id = a.recmp_map_rec and c.rec_is_delete = 0 and c.rec_is_disabled = 0 and c.rec_is_drafts = 0
	...
	function getAllRecruitmentCountry($country) {
		$sql = "select a.*,b.*,count(a.recmp_id) as numjob
				from recruitment_map_province a
				left join province b on b.province_id = a.recmp_map_province  and b.province_is_delete = 0
                left join recruitment c on c.rec_id = a.recmp_map_rec and c.rec_is_delete = 0 and c.rec_is_disabled = 0 and c.rec_is_drafts = 0
