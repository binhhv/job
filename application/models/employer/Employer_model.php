...
class Employer_model extends CI_Model {
	...
	public function getInfoEmployer($id) {
		$sql = "select a.*,IFNULL(d.numrecs, 0) as numrecs, IFNULL(c.numuser, 0)  as numuser,e.account_email as 	employer_admin_email,
				...
				e.account_id,e.account_email,e.account_code,e.account_is_delete,e.account_is_disabled,e.account_map_role,
				f.*, g.*,k.account_first_name,k.account_last_name
				...
				left join account k on k.account_id = g.emac_map_account and k.account_is_delete = 0
	...
	function deleteAccount($idUser) {
		$data = array(
			...
			'data' => array('account_is_delete' => true));
		return $this->dbutil->updateDb($data);
	}
	function disableAccount($idUser, $type) {
		$data = array(
			'from' => 'account',
			'where' => 'account_id = ' . $idUser,
			'data' => array('account_is_disabled' => $type));
		return $this->dbutil->updateDb($data);
	}
	function getInfoAccountEmployer($idUser) {
		$sql = "select * from account where account_is_delete = 0 and account_id = ?";
		return $this->dbutil->getOneRowQueryFromDb($sql, array($idUser));
	}
	function createAccountEmployer($data, $employerInfo) {

		$idUser = $this->dbutil->insertDb($data, 'account');
		if ($idUser) {
			$code = $this->util->General_Code('account', $idUser, 3);
			$this->util->update_Code('account', 'account_code', $code, 'account_id', $idUser);
			$dataEmployer = array(
				'emac_map_account' => $idUser,
				'emac_map_employer' => $employerInfo->employer_id,
				'emac_is_delete' => false,
				'emac_created_at' => date('Y-m-d'));
			$this->dbutil->insertDb($dataEmployer, 'employer_map_account');
		}

		return $idUser;
	}

	function updateUserEmployer($data) {
		return $this->dbutil->updateDb($data);
	}
}